...
public class SinkMysqlTest extends ConnectorTest {
  // datahub config
  ...
  private long sleepTime = 60000; // commit interval 30s
  ...
  private void CreateTable(RecordSchema schema, boolean hasPK) {
    ...
    for (Field field :schema.getFields()) {
      ...
      if (field.getType() == FieldType.TIMESTAMP && field.getNotnull()) {
        ...
        sql += field.getName() + " decimal(30,30), ";
      ...
      if (i.getName().equals("pk")) {
        ...
        switch (i.getType()) {
          ...
          case BIGINT:
            ...
            break;
          case DECIMAL:
            entry.setDecimal(i.getName(), new BigDecimal(Double.valueOf(random.nextInt(100)/100.0)));
            dbRecord.values.put(i.getName(), entry.getDecimal(i.getName()).toPlainString());
            break;
          default:
            Assert.assertTrue(false);
        }
    ...
    try {
      ...
      while (rs.next()) {
        ...
        for (Map.Entry<String, String> entry : dbRecord.values.entrySet()) {
          ...
          } else if (schema.getField(entry.getKey()).getType() == FieldType.DECIMAL) {
            String val = rs.getString(entry.getKey());
            Assert.assertEquals(
                    new BigDecimal(val), new BigDecimal(entry.getValue()).setScale(30, BigDecimal.ROUND_HALF_UP));
  ...
  public void testSinkMysqlConnectorNormal() {
    ...
    schema.addField(new Field("f7", FieldType.DECIMAL));
    ...
    client.createDataConnector(projectName, topicName, connectorType, columnFields, desc);
    ...
    for (ShardContext sc : getDataConnectorResult.getShardContexts()) {
      commit_count = commit_count + sc.getCurSequence() + 1;
  ...
  /**
   * TODO: Open after append connector field ready
   */
  @Test(enabled = false)
  public void testSinkMysqlConnectorNormalAppendConnectorField() {
    RecordSchema schema = new RecordSchema();
    schema.addField(new Field("f1", FieldType.STRING));
    schema.addField(new Field("f2", FieldType.BIGINT));
    schema.addField(new Field("f3", FieldType.DOUBLE));
    ...
    client.createDataConnector(projectName, topicName, connectorType, columnFields, desc);
    schema.addField(new Field("f6", FieldType.BIGINT));
    client.appendField(new AppendFieldRequest(projectName, topicName, new Field("f6", FieldType.BIGINT)));
    String sql = "alter table " + desc.getTable() + " ADD COLUMN f6 bigint ";
    execute(dbConn, sql);
    client.appendDataConnectorField(new AppendDataConnectorFieldRequest(projectName, topicName, ConnectorType.SINK_MYSQL, "f6"));
    putRecords(schema);
    try {
      Thread.sleep(sleepTime);
    } catch (InterruptedException e) {
  ...
  public void testSinkMysqlConnectorNormalAppendConnectorFieldNotNull() {
    RecordSchema schema = new RecordSchema();
    schema.addField(new Field("f1", FieldType.STRING));
    schema.addField(new Field("f2", FieldType.BIGINT));
    schema.addField(new Field("f3", FieldType.DOUBLE));
    ...
    schema.addField(new Field("f6", FieldType.BIGINT));
    client.appendField(new AppendFieldRequest(projectName, topicName, new Field("f6", FieldType.BIGINT)));
    String sql = "alter table " + desc.getTable() + " ADD COLUMN f6 bigint not null";
    execute(dbConn, sql);
    client.appendDataConnectorField(new AppendDataConnectorFieldRequest(projectName, topicName, ConnectorType.SINK_MYSQL, "f6"));
  }
