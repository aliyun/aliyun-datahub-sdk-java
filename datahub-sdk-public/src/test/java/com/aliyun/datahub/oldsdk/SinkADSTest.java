...
public class SinkADSTest extends ConnectorTest {
  // datahub config
  ...
  private void putNull(RecordSchema schema) {
    List<String> ignoreList = new ArrayList<String>();
    for (Field field :schema.getFields()) {
        ignoreList.add(field.getName());
    }
    putRecords(schema, ignoreList);
  }
  ...
  @Test
  public void testSinkADSConnectorWithNullPk() {
    RecordSchema schema = new RecordSchema();
    schema.addField(new Field("f1", FieldType.STRING));
    schema.addField(new Field("f2", FieldType.BIGINT));
    schema.addField(new Field("pk", FieldType.STRING));
    ...
    CreateTable(schema, true, 1, true);
    List<String> columnFields = new ArrayList<String>();
    columnFields.add("f1");
    columnFields.add("f2");
    columnFields.add("pk");
