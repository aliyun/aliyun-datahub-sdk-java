...
public class AppendFieldTest {
    ...
    public void tearDown() {
        try {
            for (String connector : client.listDataConnector(projectName, topicName).getDataConnectors()) {
                client.deleteDataConnector(projectName, topicName, ConnectorType.valueOf(connector.toUpperCase()));
            }
    ...
    @Test
    public void testCreateConnectorAfterAppendFieldWithDirtyData() {
        List<String> columnFields = new ArrayList<String>();
        columnFields.add("f1");
        columnFields.add("f2");
        columnFields.add("f3");
        ...
        schema.addField(new Field("f3", FieldType.STRING));
        schema.addField(new Field("f4", FieldType.STRING));
        CreateTopic(schema);

        TableSchema odpsSchema = new TableSchema();
        odpsSchema.addColumn(new Column("f1", OdpsType.STRING));
        odpsSchema.addColumn(new Column("f2", OdpsType.STRING));
        odpsSchema.addColumn(new Column("f3", OdpsType.STRING));
        odpsSchema.addColumn(new Column("f4", OdpsType.STRING));
        odpsSchema.addPartitionColumn(new Column("pt", OdpsType.STRING));
        CreateTable(odpsSchema);
        ...
        client.appendField(new AppendFieldRequest(projectName, topicName, new Field("pt", FieldType.STRING)));
        ...
        Assert.assertEquals(getDataConnectorResult.getColumnFields(), columnFields);
        for (ShardContext sc : getDataConnectorResult.getShardContexts()) {
            Assert.assertEquals(sc.getCurSequence(), recordNum*2);
        }
    }
}
