...
public class RecordEntryTest {
    ...
    public void testGetRecordSize() {
        ...
        long size = 59; // {"Data":[null,null,null,null,null,null,],"Attributes":{},},
        Assert.assertEquals(size, entry.getRecordSize());

        entry.setString(0, null);

        Assert.assertEquals(size, entry.getRecordSize());

        long bigint = DatahubTestUtils.getRandomNumber();
        size = size - 5 + String.valueOf(bigint).length() + 3;
        entry.setBigint(1, bigint);
        Assert.assertEquals(size, entry.getRecordSize());

        long timestamp = DatahubTestUtils.getRandomTimestamp();
        size = size - 5 + String.valueOf(timestamp).length() + 3;
        entry.setTimeStamp(2, timestamp);
        Assert.assertEquals(size, entry.getRecordSize());

        double decimal = DatahubTestUtils.getRandomDecimal();
        size = size - 5 + String.valueOf(decimal).length() + 3;
        entry.setDouble(3, decimal);
        Assert.assertEquals(size, entry.getRecordSize());

        boolean b = DatahubTestUtils.getRandomBoolean();
        size = size - 5 + (b ? 5 : 6);
        entry.setBoolean(4, b);
        Assert.assertEquals(size, entry.getRecordSize());

        String s = DatahubTestUtils.getRandomString(7);
        size = size - 5 + s.length() + 3;
        entry.setString(5, s);
        Assert.assertEquals(size, entry.getRecordSize());
    }
